...
public final class AndroidHttpClient implements HttpClient {
	...
	private static Logger LOG = LoggerProvider.getInstance().getLogger(AndroidHttpClient.class.getName());
	...
	private static class LoggingConfiguration {
		...
		private boolean isLoggable() {
			return LOG.isEnabled(translateLevel(level));
		}
		...
		private boolean isAuthLoggable() {
			String secure = System.getProperty("ro.secure");
			return "0".equals(secure) && LOG.isEnabled(translateLevel(level));
		}
		...
		private void println(String message) {
			LOG.log(translateLevel(level), message);
		}
	}
	...
	private static LogLevel translateLevel(int level) {
		switch (level) {
		case 2:
