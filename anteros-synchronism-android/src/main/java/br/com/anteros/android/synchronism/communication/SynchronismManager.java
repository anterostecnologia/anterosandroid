...
public class SynchronismManager implements SynchronismTransactionListener, MobileSendDataListener {
	...
	public void exportTableData(boolean executeImport) throws Exception {
		...
		if (tableListToExport != null) {
			for (String tableName : tableListToExport)
				exportTables.add(new TableExport(tableName, tableName));
		}
		...
		try {
			if (tableExport != null) {
				if (tableExport.requests == null) {
					...
					if (tableExport.tableName.equals(START_TRANSACTION)) {
						 ...
						 * servidor
						 */
					} else if (tableExport.tableName.equals(END_TRANSACTION)) {
