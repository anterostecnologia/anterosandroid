 ...
 ******************************************************************************/
package br.com.anteros.android.log.impl;
 ...
 * 
 */
public class LogCatLoggerProvider extends LoggerProvider {

	private final LogLevel level = findLevel(AnterosCoreProperties.ANTEROS_LOG_LEVEL);

	@Override
	public Logger getLogger(String name) {
		return new LogCatLogger(name, level);
	}
	...
	public static LogLevel findLevel(String propertyValue) {
		try {
			...
			System.out.println("logcatLogLevel: " + consoleLogLevel);
			return LogLevel.valueOf(consoleLogLevel);
		} catch (Exception e) {
			System.err.println("Level de configuração do LogCat não foi configurado: " + e.getMessage());
	...
	public Logger getLogger(Class clazz) {
		return new LogCatLogger(clazz.getSimpleName(), level);
	}
