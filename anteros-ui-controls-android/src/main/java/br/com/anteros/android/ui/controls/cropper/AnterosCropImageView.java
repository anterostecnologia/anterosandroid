...
package br.com.anteros.android.ui.controls.cropper;
 ...
 */
public class AnterosCropImageView extends FrameLayout {
     ...
     */
    private final AnterosCropOverlayView mCropOverlayView;
     ...
     */
    private AnterosCropImageAnimation mAnimation;
    ...
    public AnterosCropImageView(Context context) {
        this(context, null);
    }

    public AnterosCropImageView(Context context, AttributeSet attrs) {
        super(context, attrs);

        AnterosCropImageOptions options = null;
        Intent intent = context instanceof Activity ? ((Activity) context).getIntent() : null;
        if (intent != null) {
            Bundle bundle = intent.getBundleExtra(AnterosCropImageOptions.BUNDLE_KEY);
            if (bundle != null) {
                options = bundle.getParcelable(AnterosCropImage.CROP_IMAGE_EXTRA_OPTIONS);
            }
        }

        if (options == null) {

            options = new AnterosCropImageOptions();

            if (attrs != null) {
                TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AnterosCropImageView, 0, 0);
                try {
                    options.fixAspectRatio = ta.getBoolean(R.styleable.AnterosCropImageView_cropFixAspectRatio, options.fixAspectRatio);
                    options.aspectRatioX = ta.getInteger(R.styleable.AnterosCropImageView_cropAspectRatioX, options.aspectRatioX);
                    options.aspectRatioY = ta.getInteger(R.styleable.AnterosCropImageView_cropAspectRatioY, options.aspectRatioY);
                    options.scaleType = ScaleType.values()[ta.getInt(R.styleable.AnterosCropImageView_cropScaleType, options.scaleType.ordinal())];
                    options.autoZoomEnabled = ta.getBoolean(R.styleable.AnterosCropImageView_cropAutoZoomEnabled, options.autoZoomEnabled);
                    options.multiTouchEnabled = ta.getBoolean(R.styleable.AnterosCropImageView_cropMultiTouchEnabled, options.multiTouchEnabled);
                    options.maxZoom = ta.getInteger(R.styleable.AnterosCropImageView_cropMaxZoom, options.maxZoom);
                    options.cropShape = CropShape.values()[ta.getInt(R.styleable.AnterosCropImageView_cropShape, options.cropShape.ordinal())];
                    options.guidelines = Guidelines.values()[ta.getInt(R.styleable.AnterosCropImageView_cropGuidelines, options.guidelines.ordinal())];
                    options.snapRadius = ta.getDimension(R.styleable.AnterosCropImageView_cropSnapRadius, options.snapRadius);
                    options.touchRadius = ta.getDimension(R.styleable.AnterosCropImageView_cropTouchRadius, options.touchRadius);
                    options.initialCropWindowPaddingRatio = ta.getFloat(R.styleable.AnterosCropImageView_cropInitialCropWindowPaddingRatio, options.initialCropWindowPaddingRatio);
                    options.borderLineThickness = ta.getDimension(R.styleable.AnterosCropImageView_cropBorderLineThickness, options.borderLineThickness);
                    options.borderLineColor = ta.getInteger(R.styleable.AnterosCropImageView_cropBorderLineColor, options.borderLineColor);
                    options.borderCornerThickness = ta.getDimension(R.styleable.AnterosCropImageView_cropBorderCornerThickness, options.borderCornerThickness);
                    options.borderCornerOffset = ta.getDimension(R.styleable.AnterosCropImageView_cropBorderCornerOffset, options.borderCornerOffset);
                    options.borderCornerLength = ta.getDimension(R.styleable.AnterosCropImageView_cropBorderCornerLength, options.borderCornerLength);
                    options.borderCornerColor = ta.getInteger(R.styleable.AnterosCropImageView_cropBorderCornerColor, options.borderCornerColor);
                    options.guidelinesThickness = ta.getDimension(R.styleable.AnterosCropImageView_cropGuidelinesThickness, options.guidelinesThickness);
                    options.guidelinesColor = ta.getInteger(R.styleable.AnterosCropImageView_cropGuidelinesColor, options.guidelinesColor);
                    options.backgroundColor = ta.getInteger(R.styleable.AnterosCropImageView_cropBackgroundColor, options.backgroundColor);
                    options.showCropOverlay = ta.getBoolean(R.styleable.AnterosCropImageView_cropShowCropOverlay, mShowCropOverlay);
                    options.showProgressBar = ta.getBoolean(R.styleable.AnterosCropImageView_cropShowProgressBar, mShowProgressBar);
                    options.borderCornerThickness = ta.getDimension(R.styleable.AnterosCropImageView_cropBorderCornerThickness, options.borderCornerThickness);
                    options.minCropWindowWidth = (int) ta.getDimension(R.styleable.AnterosCropImageView_cropMinCropWindowWidth, options.minCropWindowWidth);
                    options.minCropWindowHeight = (int) ta.getDimension(R.styleable.AnterosCropImageView_cropMinCropWindowHeight, options.minCropWindowHeight);
                    options.minCropResultWidth = (int) ta.getFloat(R.styleable.AnterosCropImageView_cropMinCropResultWidthPX, options.minCropResultWidth);
                    options.minCropResultHeight = (int) ta.getFloat(R.styleable.AnterosCropImageView_cropMinCropResultHeightPX, options.minCropResultHeight);
                    options.maxCropResultWidth = (int) ta.getFloat(R.styleable.AnterosCropImageView_cropMaxCropResultWidthPX, options.maxCropResultWidth);
                    options.maxCropResultHeight = (int) ta.getFloat(R.styleable.AnterosCropImageView_cropMaxCropResultHeightPX, options.maxCropResultHeight);
                    options.flipHorizontally = ta.getBoolean(R.styleable.AnterosCropImageView_cropFlipHorizontally, options.flipHorizontally);
                    options.flipVertically = ta.getBoolean(R.styleable.AnterosCropImageView_cropFlipHorizontally, options.flipVertically);

                    mSaveBitmapToInstanceState = ta.getBoolean(R.styleable.AnterosCropImageView_cropSaveBitmapToInstanceState, mSaveBitmapToInstanceState);

                    // if aspect ratio is set then set fixed to true
                    if (ta.hasValue(R.styleable.AnterosCropImageView_cropAspectRatioX) &&
                            ta.hasValue(R.styleable.AnterosCropImageView_cropAspectRatioX) &&
                            !ta.hasValue(R.styleable.AnterosCropImageView_cropFixAspectRatio)) {
        ...
        mCropOverlayView = (AnterosCropOverlayView) v.findViewById(R.id.CropOverlayView);
        mCropOverlayView.setCropWindowChangeListener(new AnterosCropOverlayView.CropWindowChangeListener() {
            @Override
                ...
                if (newZoom > 0 && newZoom != mZoom) {
                    if (animate) {
                        if (mAnimation == null) {
                            ...
                            mAnimation = new AnterosCropImageAnimation(mImageView, mCropOverlayView);
                        }
    ...
    public interface OnSetImageUriCompleteListener {
         ...
         */
        void onSetImageUriComplete(AnterosCropImageView view, Uri uri, Exception error);
    }
    //endregion
    ...
    public interface OnCropImageCompleteListener {
         ...
         */
        void onCropImageComplete(AnterosCropImageView view, CropResult result);
    }
    //endregion
