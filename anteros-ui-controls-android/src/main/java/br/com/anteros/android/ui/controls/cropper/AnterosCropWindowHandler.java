 ...
 */
final class AnterosCropWindowHandler {
     ...
     */
    public void setInitialAttributeValues(AnterosCropImageOptions options) {
     ...
     */
    public AnterosCropWindowMoveHandler getMoveHandler(float x, float y, float targetRadius, AnterosCropImageView.CropShape cropShape) {
        AnterosCropWindowMoveHandler.Type type = cropShape == AnterosCropImageView.CropShape.OVAL
                ? getOvalPressedMoveType(x, y)
                : getRectanglePressedMoveType(x, y, targetRadius);
        return type != null ? new AnterosCropWindowMoveHandler(type, this, x, y) : null;
    }
     ...
     * @return the Handle that was pressed; null if no Handle was pressed
     */
    private AnterosCropWindowMoveHandler.Type getRectanglePressedMoveType(float x, float y, float targetRadius) {
        AnterosCropWindowMoveHandler.Type moveType = null;

        // Note: corner-handles take precedence, then side-handles, then center.
        if (AnterosCropWindowHandler.isInCornerTargetZone(x, y, mEdges.left, mEdges.top, targetRadius)) {
            moveType = AnterosCropWindowMoveHandler.Type.TOP_LEFT;
        } else if (AnterosCropWindowHandler.isInCornerTargetZone(x, y, mEdges.right, mEdges.top, targetRadius)) {
            moveType = AnterosCropWindowMoveHandler.Type.TOP_RIGHT;
        } else if (AnterosCropWindowHandler.isInCornerTargetZone(x, y, mEdges.left, mEdges.bottom, targetRadius)) {
            moveType = AnterosCropWindowMoveHandler.Type.BOTTOM_LEFT;
        } else if (AnterosCropWindowHandler.isInCornerTargetZone(x, y, mEdges.right, mEdges.bottom, targetRadius)) {
            moveType = AnterosCropWindowMoveHandler.Type.BOTTOM_RIGHT;
        } else if (AnterosCropWindowHandler.isInCenterTargetZone(x, y, mEdges.left, mEdges.top, mEdges.right, mEdges.bottom) && focusCenter()) {
            moveType = AnterosCropWindowMoveHandler.Type.CENTER;
        } else if (AnterosCropWindowHandler.isInHorizontalTargetZone(x, y, mEdges.left, mEdges.right, mEdges.top, targetRadius)) {
            moveType = AnterosCropWindowMoveHandler.Type.TOP;
        } else if (AnterosCropWindowHandler.isInHorizontalTargetZone(x, y, mEdges.left, mEdges.right, mEdges.bottom, targetRadius)) {
            moveType = AnterosCropWindowMoveHandler.Type.BOTTOM;
        } else if (AnterosCropWindowHandler.isInVerticalTargetZone(x, y, mEdges.left, mEdges.top, mEdges.bottom, targetRadius)) {
            moveType = AnterosCropWindowMoveHandler.Type.LEFT;
        } else if (AnterosCropWindowHandler.isInVerticalTargetZone(x, y, mEdges.right, mEdges.top, mEdges.bottom, targetRadius)) {
            moveType = AnterosCropWindowMoveHandler.Type.RIGHT;
        } else if (AnterosCropWindowHandler.isInCenterTargetZone(x, y, mEdges.left, mEdges.top, mEdges.right, mEdges.bottom) && !focusCenter()) {
            moveType = AnterosCropWindowMoveHandler.Type.CENTER;
        }
     ...
     * @return the Handle that was pressed; null if no Handle was pressed
     */
    private AnterosCropWindowMoveHandler.Type getOvalPressedMoveType(float x, float y) {
        ...
        */
        ...
        AnterosCropWindowMoveHandler.Type moveType;
        if (x < leftCenter) {
            if (y < topCenter) {
                moveType = AnterosCropWindowMoveHandler.Type.TOP_LEFT;
